...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        new_braches = [replace_leaf(i, old, new) for i in branches(t)]
    return tree(label(t), new_braches)
...
def is_weight(w):
    ...
    "*** YOUR CODE HERE ***"
    return is_tree(w) and branches(w) == []
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        left, right = [s for s in sides(m)]
        if length(left) * total_weight(end(left)) == length(right) * total_weight(end(right)):
            return min([balanced(end(left)), balanced(end(right))])
        else:
            return False
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    lst, locked = [], False
    def protect(amount, passphrase):
        nonlocal balance, lst, locked
        if not locked:
