...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        def for_mapping(fn,cur_env):
        	def pass_in_mapping(expr):
        		return fn(expr,cur_env)
        	return pass_in_mapping

        procedure_first = scheme_eval(first,env)
        check_procedure(procedure_first)
        #if not isinstance(procedure,MacroProcedure):
        #    print('eval')

        return scheme_apply(procedure_first,rest.map(for_mapping(scheme_eval,env)),env)
        #else:
        #    print('macro')
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals is not nil or vals is not nil:
            raise SchemeError('Wrong number of aruguments passed in')
        # END PROBLEM 11
        return child
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result,Thunk):
            #print(result.expr)
            if scheme_symbolp(result.expr) or self_evaluating(result.expr):
                ...
                if scheme_symbolp(first) and first in SPECIAL_FORMS:
                    ...
                    result = SPECIAL_FORMS[first](rest, result.env)
                else:
                    
                    procedure = scheme_eval(first,result.env)
                    if not isinstance (procedure, MacroProcedure):
                        result = scheme_apply(procedure,rest.map(for_mapping(scheme_eval,result.env)), result.env)
                    else:
                        #print(rest)
                        result = scheme_eval(procedure.apply_macro(rest,result.env),result.env)
                        #result = scheme_eval(scheme_apply(procedure,rest,result.env),result.env)
