...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def commentary(score0, score1):
        if who == 0:
            return announce_highest(who, high(0, score0), score0)
        else:
            return announce_highest(who, high(1, score1), score1)

    def high(who, score):
        current_high = score - previous_score
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    dice_num, high = 1, 0
    while dice_num < 11:
        current = make_averaged(roll_dice, num_samples)(dice_num, dice)
        if current > high:
            high = current
            result = dice_num
        dice_num += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if (is_swap(current, opponent_score) and current < opponent_score) or free_bacon(opponent_score) >= margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    We apply the swap_strategy to achieve a high win rate against the always_roll(4)
    """
    # BEGIN PROBLEM 12
