...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and self.ant is insect:
                if insect is not self.ant.queen_group[0]:
                    self.ant = None
                    insect.place = None
            elif isinstance(insect, QueenAnt) and self.ant.ant and self.ant.ant is insect:
                if insect is not self.ant.ant.queen_group[0]:
                    self.ant.ant = None
                    insect.place = None
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        attacked = random_or_none(self.place.bees)
        c_range = self.min_range

        while not attacked and self.place.entrance is not hive and c_range < self.max_range:
            self.place = self.place.entrance
            attacked = random_or_none(self.place.bees)
            c_range += 1
...
# END Problem 13
            ...
            while self.place.exit is not None:
                ...
                if self.place.ant is not None:
                    if self.place.ant.container:
                        if self.place.ant.ant is not None and self.place.ant.ant not in self.doubled:
                            self.place.ant.ant.damage *= 2
                            self.doubled.append(self.place.ant.ant)
                        if self.place.ant not in self.doubled:
                            self.place.ant.damage *= 2
                            self.doubled.append(self.place.ant)
                    else:
                        if self.place.ant not in self.doubled:
                            self.place.ant.damage *= 2
                            self.doubled.append(self.place.ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self is self.queen_group[0] and not self.armor:
            bees_win()
        # END Problem 13
