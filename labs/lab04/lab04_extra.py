...
def flatten(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(lst, indicator = 0, result = []):
        if indicator == len(lst) - 1:
            if type(lst[indicator]) != list:
                return result + [lst[indicator]]
            else:
                return result + lst[indicator]
        elif type(lst[indicator]) != list:
            return helper(lst, indicator + 1, result + [lst[indicator]])
        else:
            return helper(lst, indicator + 1, helper(result + lst[indicator]))
    return helper(lst)
