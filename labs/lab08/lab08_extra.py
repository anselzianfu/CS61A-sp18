...
def trade(first, second):
    ...
    for m in range(0, len(first)):
        for n in range(0, len(second)):
            if sum(first[:m]) == sum(second[:n]) and first[:m]:
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def personal(lst):
        if lst is Link.empty:
            ...
            result = ''
            while lst.rest:
                result += front + str(lst.first) + mid
                lst = lst.rest
                count += 1
            result += front + str(lst.first) + mid + empty_repr
